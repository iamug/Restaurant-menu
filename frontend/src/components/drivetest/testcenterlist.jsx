...
const TestCenterListComponent = (props) => {
  ...
  let [testCenterData, setTestCenterData] = useState([]);
  ...
  const deletetestcenter = async (id) => {
    ...
    let onOk = async () => {
      try {
        ...
        const res = await API.delete("/api/testcenters/" + id, config);
        if (!res) {
          notifier.alert("Error. Kindly check internet connection");
          return false;
        }
  ...
  const addtestcenter = async () => {
    ...
    try {
      ...
      const res = await API.post("/api/testcenters", body, config);
      if (res.status == 201) {
        new AWN().success("Test center added successfully ", {
  ...
  const updatetestcenter = async (id) => {
    const { isEnabled, name, address, slotsPerDay } = formValue;
    if (!name && !address && !slotsPerDay && isEnabled === undefined) {
      new AWN().alert("Kindly fill all fields", {
        durations: { alert: 4000 },
    ...
    try {
      ...
      const res = await API.put("/api/testcenters/" + id, body, config);
      if (res.status == 200) {
        new AWN().success("Test center updated successfully ", {
          durations: { success: 3000 },
        });
        setRefreshData(!refreshData);
      } else {
  ...
  useEffect(async () => {
    ...
    const testcenters = await fetchtestcenters();
    if (!testcenters) {
            ...
            <div className="row">
              <div className="col-12">
                <div className="page-title-box">
                  <h4 className="page-title">Test Centers</h4>
                </div>
              </div>
            </div>
            {/* end page title */}
                  ...
                  <div className="card-body">
                    ...
                    <div className="table-responsive">
                      <table className="table table-centered table-nowrap table-hover mb-0">
                        ...
                        <tbody>
                          {testCenterData && testCenterData.length >= 1 ? (
                            testCenterData.map((testcenter, index) => (
                              <>
                                <tr key={index}>
                            ...
                            testCenterData.length === 0 &&
                            loading ? (
                            <tr>
                              <td colSpan={6} className="text-center py-5">
                                <h3> There are no Test Centers</h3>
                              </td>
                            </tr>
                          ) : (
                            <tr>
        ...
        <Drawer.Body>
          <Form
            ...
            onSubmit={(data) => {
              ...
              dataUpdate
                ? updatetestcenter(formValue.testCenterId)
                : addtestcenter();
            }}
            onChange={(data) => {
              console.log({ data });
...
export default TestCenterListComponent;
