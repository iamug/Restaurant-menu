...
const SOSComponent = (props) => {
  ...
  const deleterequest = async (id) => {
    ...
    let onOk = async () => {
      try {
        ...
        const res = await API.delete("/api/reportsos/" + id, config);
        if (!res) {
          notifier.alert("Error. Kindly check internet connection");
          return false;
        }
  ...
  const updaterequest = async (id) => {
    ...
    try {
      ...
      const res = await API.put("/api/reportsos/" + id, body, config);
      if (res.status == 200) {
  ...
  const fetchrequests = async () => {
    try {
      ...
      const res = await API.get("/api/reportsos", config);
      if (!res) return false;
      return res.data;
    } catch (err) {
      console.log(err);
            ...
            <div className="row">
              <div className="col-12">
                <div className="page-title-box">
                  <h4 className="page-title">Report SOS</h4>
                </div>
              </div>
            </div>
            {/* end page title */}
                          ...
                          {requestData && requestData.length >= 1 ? (
                            requestData.map((request, index) => (
                              <tr key={index}>
                                ...
                                <td>{request.issuerPhone}</td>
                                <td>{request.issuer}</td>
                                <td>
                ...
                <Col xs={12}>
                  <Grid fluid>
                    ...
                    <Row gutter={10}>
                      <Col xs={12}>
                        <FormGroup>
                          <ControlLabel>Issuer Phone </ControlLabel>
                          <FormControl name="issuerPhone" disabled />
                        </FormGroup>
                      </Col>
                      <Col xs={12}></Col>
                    </Row>
                  ...
                  {formValue.mediaUrl && (
                    ...
                    >
                      ...
                      <video
                        ...
                        controls
                      >
                        <source src={formValue.mediaUrl} type="video/mp4" />
